...
//Create raw socket for ICMP request using IPv4
int create_icmp_socket() {
	int sockfd;

	sockfd = socket(AF_INET, SOCK_RAW, IPPROTO_ICMP);
    if (sockfd < 0) {
		if (DEBUG_LEVEL >= LOW) printf("[ICMP]Failed to create socket\n"); 
        perror("Raw socket creation failed");
        return sockfd;
    }
    if (DEBUG_LEVEL >= MEDIUM) printf("[ICMP]Socket created\n"); 
	return sockfd;
}

void print_buffer_hex(const unsigned char *buffer, size_t size) {
    for (size_t i = 0; i < size; i++) {
        printf("%02X ", buffer[i]);  // Affichage en majuscules (ex: "0A 1F 2C")
    }
    printf("\n");
}

//Wait and read the ICMP response
